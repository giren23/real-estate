...
def normalize_payload(rows: list[dict], collected_at: str) -> dict:
    if len(rows) < 100:
    ...
    for row in rows:
        ...
        value = row.get("dtaVal")
        if not name or not code or not isinstance(value, (int, float)):
            continue
        normalized.append({
...
def fetch_payload() -> dict:
    ...
    if not isinstance(rows, list):
        raise ValueError("R-ONE 지역별 응답에 data 배열이 없음")
    return normalize_payload(rows, datetime.now().astimezone().isoformat(timespec="seconds"))
...
def snapshot_is_fresh(max_age_hours: int = 18) -> bool:
    ...
    try:
        ...
        return datetime.now().astimezone() - collected_at < timedelta(hours=max_age_hours)
    except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError):
        return False


def main() -> None:
    parser = argparse.ArgumentParser(description="한국부동산원 R-ONE 지역별 월간 변동률을 안전하게 갱신합니다.")
    parser.add_argument("--force", action="store_true", help="18시간 이내에 갱신했더라도 다시 확인")
    args = parser.parse_args()
    if not args.force and snapshot_is_fresh():
        print("R-ONE 지역 변동률은 18시간 이내 검증된 스냅샷이 있어 갱신을 건너뜁니다.")
        return
    ...
    city_count = sum(len(row["cities"]) for row in payload["provinces"])
    print(f"R-ONE 지역 변동률 저장 완료: {payload['period']}, 시도 {len(payload['provinces'])}곳, 시군구 {city_count}곳")
