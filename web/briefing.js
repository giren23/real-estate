(() => {
  ...
  const orderedSections = sections => {
    const preferredOrder = ["verdict", "us", "kr", "sectors", "kr-top5", "us-top5", "hynix", "risk", "action", "core", "events", "world"];
  ...
  const newsHtml = news => (news || []).map(item => `<article><time>${esc(item.publisher || "출처 미상")}${item.important ? " · [중요]" : ""}</time><h3>${item.url ? `<a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${esc(item.title)}</a>` : esc(item.title)}</h3>${item.url ? `<a class="brief-source-link" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">원문 보기 →</a>` : ""}</article>`).join("");
  const detailHtml = section => (section.details || []).map(paragraph => `<p class="brief-detail">${esc(paragraph)}</p>`).join("");
  const rankingsHtml = rankings => (rankings || []).map(group => `<div class="brief-rankings"><article><b>${esc(group.market || "관심 종목")}</b><ol>${(group.items || []).map(item => `<li><strong>${esc(item.name)}</strong><span>${esc(item.description)}</span><small>${esc(item.reason)}</small></li>`).join("")}</ol></article></div>`).join("");
  const renderSection = section => `<section class="brief-card"><div class="brief-card-head"><div><h2>${esc(section.title)}</h2><p>${esc(section.subtitle || "핵심 총평")}</p></div></div><p class="brief-section-summary">${esc(section.summary || "자료 준비 중")}</p>${detailHtml(section)}${rankingsHtml(section.rankings)}${(section.metrics || []).length ? `<div class="brief-metrics">${section.metrics.map(metricHtml).join("")}</div>` : ""}${(section.checks || []).length ? `<ul class="brief-checks">${section.checks.map(check => `<li>${esc(check)}</li>`).join("")}</ul>` : ""}${(section.scenarios || []).length ? `<div class="brief-scenarios">${section.scenarios.map(scenario => `<article><span>${esc(scenario.label)}</span><b>${esc(scenario.title)}</b><p>${esc(scenario.body)}</p></article>`).join("")}</div>` : ""}${(section.news || []).length ? `<div class="brief-news">${newsHtml(section.news)}</div>` : ""}</section>`;
  const render = data => {
