...
function renderResults(matches,query){
  ...
  byId("results").innerHTML=matches.map(item=>{
    ...
    return '<div class="result" data-key="'+esc(g.key)+'"><div class="result-top"><div><h3>'+esc(g.apt_name)+'</h3><p>'+esc(addressOf(g))+'</p></div><span class="price">'+price+'</span></div><div class="result-actions"><span>'+distance+' · '+areas+'</span><button class="add-btn" type="button" '+(added?"disabled":"")+'>'+(added?"추가됨":"비교 추가")+"</button></div></div>";
  }).join("");
...
function addSelection(group){
  ...
  const popular=[...group.areas].sort((a,b)=>group.trades.filter(r=>r.area_m2===b).length-group.trades.filter(r=>r.area_m2===a).length)[0]||0;
...
function renderSelections(){
  ...
  byId("selections").innerHTML=selected.map((item,index)=>{
    const g=item.group;
    return '<div class="selection" data-key="'+esc(item.key)+'"><div class="selection-name"><b><i style="background:'+palette[index]+'"></i>'+esc(g.apt_name)+'</b><span>'+esc(g.region_name+" "+g.dong)+'</span></div><select aria-label="'+esc(g.apt_name)+' 평형 선택">'+(g.areas.length?g.areas.map(a=>'<option value="'+a+'" '+(a===item.area?"selected":"")+'>전용 '+fmt(a)+'㎡ ('+fmt(a/3.3058)+'평)</option>').join(""):'<option value="0">평형 거래자료 없음</option>')+'</select><button class="remove-btn" type="button" aria-label="'+esc(g.apt_name)+' 삭제">삭제</button></div>';
  }).join("");
...
function renderComparison(){
  const datasets=selected.map((item,index)=>{
    let points=item.group.history.filter(r=>Number(r.area_m2)===item.area).map(r=>({x:r.month,y:r.median_price_eok}));
    if(!points.length){
      const daily=new Map();
      item.group.trades.filter(r=>Number(r.area_m2)===item.area).forEach(r=>{if(!daily.has(r.trade_date))daily.set(r.trade_date,[]);daily.get(r.trade_date).push(r.price_eok);});
      points=[...daily].map(([x,values])=>({x,y:median(values)}));
    }
    return {label:item.group.apt_name+" · "+(item.area?fmt(item.area)+"㎡":"평형 없음"),data:points,borderColor:palette[index],backgroundColor:palette[index],pointRadius:3,pointHoverRadius:6,tension:.2,spanGaps:true};
  });
  if(priceChart) priceChart.destroy();
  priceChart=new Chart(byId("priceChart"),{type:"line",data:{datasets},options:{maintainAspectRatio:false,responsive:true,parsing:false,interaction:{mode:"nearest",intersect:false},scales:{x:{type:"category",title:{display:true,text:"거래월"}},y:{title:{display:true,text:"실거래가 (억원)"},beginAtZero:false}},plugins:{legend:{position:"bottom",labels:{usePointStyle:true,boxWidth:8}},tooltip:{callbacks:{label:c=>c.dataset.label+": "+fmt(c.raw.y)+"억원"}}}}});
}
